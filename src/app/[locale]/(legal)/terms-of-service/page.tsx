...
export async function generateMetadata(
  ...
  const pageData = await getPageData('terms-of-service')
  
  const title = pageData?.[`title_${currentLocale}`] || pageData?.title || 'Terms of Service | Coriyon Arrington'
  const description = pageData?.[`meta_description`] || 'Terms of Service for CoriyonsList and Fearfully Forged LLC.'
    ...
    <Stack gap="0" w="full">
      <Box className="pattern-dots" borderTopWidth="1px" borderColor="border.subtle" bg="bg.canvas">
        <Container maxW="7xl" px={{ base: "4", md: "8" }}>
          <FadeIn>
            <CenteredHeroLayout 
              badge={<Badge size="lg" colorPalette="gray" variant="subtle" rounded="full" px="3" py="1">{isEs ? "Legal" : "Legal"}</Badge>}
              title={isEs ? (pageData?.title_es || "Términos de Servicio") : (pageData?.title || "Terms of Service")}
              description={isEs ? "Las reglas, pautas y acuerdos para usar mis servicios." : "The rules, guidelines, and agreements for using my services."}
              pb={{ base: "12", md: "16" }}
            />
          </FadeIn>
        </Container>
