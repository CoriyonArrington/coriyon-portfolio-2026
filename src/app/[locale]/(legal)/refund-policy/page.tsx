...
export async function generateMetadata(
  ...
  const pageData = await getPageData('refund-policy')
  
  const title = pageData?.[`title_${currentLocale}`] || pageData?.title || 'Refund Policy | Coriyon Arrington'
  const description = pageData?.[`meta_description`] || 'Refund Policy for digital products and services.'
        ...
        <Container maxW="7xl" px={{ base: "4", md: "8" }}>
          <FadeIn>
            <CenteredHeroLayout 
              ...
              title={isEs ? (pageData?.title_es || "Política de Reembolso") : (pageData?.title || "Refund Policy")}
              description={isEs ? "Mi compromiso de garantizar su satisfacción." : "My commitment to ensuring your satisfaction."}
              pb={{ base: "12", md: "16" }}
            />
          </FadeIn>
        </Container>
