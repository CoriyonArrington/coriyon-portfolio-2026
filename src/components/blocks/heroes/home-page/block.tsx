...
import { Badge, Box, Button, Center, Icon, Stack, chakra, Dialog, Portal, IconButton, HStack, Text, Avatar, Skeleton, VStack, Container, Heading } from '@chakra-ui/react'
...
// FIX: Updated the parser to use the dynamic "colorPalette.600" CSS variable mapping
const renderHighlightedTitle = (rawText: string) => {
    ...
    <>
      {parts.map((part, index) => {
        if (part.startsWith('*') && part.endsWith('*')) {
          return (
            <Text as="span" key={index} color="colorPalette.600">
              {part.slice(1, -1)}
            </Text>
          );
        }
...
export const Block = ({ dict, title, description, tagline, videoUrl, imageUrl, introVideoUrl }: BlockProps) => {
  ...
  const ActionButtons = () => (
    <VStack gap="6" w="full" align={hasMedia ? { base: "center", lg: "flex-start" } : "center"}>
      <Stack direction={{ base: 'column', md: 'row' }} gap="4" w={{ base: "full", md: "auto" }} align={{ base: 'stretch', md: 'center' }}>
        
        {/* FIX: Primary button now uses the dynamic background CSS variable mapping */}
        <Button size="xl" h={{ base: 14, md: 16 }} px={{ base: 6, md: 8 }} fontSize="lg" bg="colorPalette.600" color="white" _hover={{ bg: "colorPalette.700" }} onClick={() => scrollTo('projects')} onMouseEnter={playHover} w={{ base: "full", md: "auto" }}>
      ...
      {displaySocialProof && (
        <HStack gap="3" onClick={() => scrollTo('testimonials')} onMouseEnter={playHover} cursor="pointer" transition="all 0.2s" _hover={{ opacity: 0.8 }} h="10">
          ...
          <Stack gap="0" pt="1" align={hasMedia ? "flex-start" : "center"}>
  ...
  const TaglineBadge = () => displayTagline ? (
  ...
  if (!hasMedia) {
    return (
      <Box py={{ base: '16', md: '24', lg: '32' }} w="full">
        <FadeIn>
          <Container maxW="3xl">
            <Stack gap={{ base: '8', md: '10' }} align="center" textAlign="center">
              <TaglineBadge />
              <Heading size={{ base: '5xl', md: '6xl', lg: '7xl' }} fontWeight="bold" letterSpacing="tight" lineHeight="1.1">
  ...
  return (
    <SplitScreenHeroLayout
      badge={<TaglineBadge />}
      title={renderHighlightedTitle(rawTitle) as unknown as string}
